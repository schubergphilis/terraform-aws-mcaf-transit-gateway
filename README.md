...
| <a name="input_cloudwatch_flow_logs_configuration"></a> [cloudwatch\_flow\_logs\_configuration](#input\_cloudwatch\_flow\_logs\_configuration) | Cloudwatch flow logs configuration | <pre>object({<br>    kms_key_arn              = optional(string)<br>    log_group_name           = optional(string, "/platform/transit-gateway-flow-logs")<br>    max_aggregation_interval = optional(number, 60)<br>    retention_in_days        = optional(number, 90)<br>    traffic_type             = optional(string, "ALL")<br>  })</pre> | `{}` | no |
| <a name="input_enable_cloudwatch_flow_logs"></a> [enable\_cloudwatch\_flow\_logs](#input\_enable\_cloudwatch\_flow\_logs) | value | `bool` | `true` | no |
| <a name="input_enable_s3_flow_logs"></a> [enable\_s3\_flow\_logs](#input\_enable\_s3\_flow\_logs) | value | `bool` | `false` | no |
